...
make sync-version
# or
python scripts/sync_version.py --entry udi-hunterdouglas-pg3x.py
```

Or bump in one step:

```bash
python scripts/bump_version.py 1.15.0 --entry udi-hunterdouglas-pg3x.py
```
...
Use **`main`** as the default branch. The gitea mirror uses `main`; the UDI store org repo may still list `master` as default until an org admin switches it.
...
```bash
cd ~/polyglot/nodeservers/udi-hunterdouglas-pg3x
make install-eisy UV_PYTHON="$UV_PYTHON"
direnv allow .
```

Run lint on macOS (or upgrade Rust on EISY and set `UV_SYNC_LINT=1`). Hunter pulls in `aiohttp`, which compiles on FreeBSD — you may need `pkg install python311 rust gmake gcc`.

## server.json GitHub URLs

| Stage | Org | Repo |
|-------|-----|------|
| Development | `sejgit-udi-plugins` | `udi-hunterdouglas-pg3x` |
| Published on UDI store | `UniversalDevicesInc-PG3` | `udi-hunterdouglas-pg3` |
