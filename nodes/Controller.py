...
class Controller(Node):
    ...
    def start(self):
        ...
        """
        LOGGER.info(
            f"Started HunterDouglas PG3 NodeServer {self.poly.serverdata['version']}"
        )
        self.Notices.clear()
    ...
    def checkParams(self):
        ...
        if self.gateway is None or (
            ...
            self.Notices["gateway"] = (
                "No gateway defined, assume G3 default; no check; best define ip address(es)"
            )
            return True
    ...
    def _goodip(self) -> bool:
        ...
        self.Notices["gateway"] = (
            "All Gateway IPs unreachable. Check 'gatewayip' in customParams"
        )
        return False
    ...
    def pollUpdate(self):
        ...
        """
        if self.poll_in:
            LOGGER.error("Still in Poll, exiting")
            return
        self.poll_in = True
    ...
    def _poll_events(self):
        ...
        while not self.stop_sse_client_event.is_set():
            ...
            if event.get("evt") == "scene-add":
                ...
                if not match:
                    LOGGER.info(
                        f"gateway event - scene-add, NEW so start Discover - {event}"
                    )
    ...
    def _create_shade_node(self, shade, shTxt, capabilities):
        ...
        cls = next(
            (cls for caps, cls in node_classes.items() if capabilities in caps), Shade
        )
        ...
        try:
            if data:
                ...
                for scene in data["scenes"]:
                    ...
                    name = scene["name"]
                    if scene["room_Id"] is None:
                        room_name = "Multi"
                    else:
                        room_name = self.rooms_map.get(scene["room_Id"], {}).get(
                            "name", "Multi"
                        )[0:ROOM_NAME_LIMIT]
    ...
    def getHomeG3(self):
        ...
        if self.gateways:
            if code == requests.codes.ok:
                data = res.json()
                LOGGER.info(
                    "getHomeG3 gateway good %s, %s", self.gateway, self.gateways
                )
                return data
            else:
                LOGGER.error(
                    "getHomeG3 gateway NOT good %s, %s", self.gateway, self.gateways
                )
        else:
    ...
    def getScenesActiveG3(self):
        ...
        if self.gateways:
            if code == requests.codes.ok:
                data = res.json()
                LOGGER.info(
                    "getScenesActiveG3 good %s, %s", self.gateway, self.gateways
                )
                return data
            else:
                LOGGER.error(
                    "getScenesActiveG3 NOT good %s, %s", self.gateway, self.gateways
                )
        else:
    ...
    def get(self, url: str) -> requests.Response:
        ...
        try:
            ...
            res.status_code = 300
            res._content = (
                b'{"errMsg": "Error fetching from gateway, check configuration"}'
            )
            return res
        ...
        LOGGER.debug(
            f"Get from '{url}' returned {res.status_code}, response body '{res.text}'"
        )
    ...
    def put(self, url: str, data: dict | None = None) -> dict | bool:
        ...
        try:
            ...
            LOGGER.debug(
                f"Put to '{url}' succeeded with status {res.status_code}, response body: {res.text}"
            )
            try:
                return res.json()
            except ValueError:
